...
uint32_t V2Memory::Firmware::getStart() {
  return (uint32_t)&__text_start__;
}

extern uint32_t __sketch_size;
uint32_t V2Memory::Firmware::getSize() {
  // The '__sketch_size' symbol requires explicit support from the linker script.
  return (uint32_t)&__sketch_size;
}

void V2Memory::Firmware::calculateHash(uint32_t offset, uint32_t len, char hash[41]) {
  V2Cryptography::SHA1 sha;
  sha.init();
...
bool V2Memory::Firmware::Secondary::verify(uint32_t firmware_len, const char *hash) {
  if (memcmp(NULL, (const uint8_t *)getStart(), Firmware::getStart()) != 0)
    return false;

  const uint32_t start = getStart() + Firmware::getStart();
  char secondary[41];
  calculateHash(start, firmware_len, secondary);
  return strcmp(hash, secondary) == 0;
}

void V2Memory::Firmware::Secondary::copyBootloader() {
...
void V2Memory::Firmware::Secondary::activate() {
