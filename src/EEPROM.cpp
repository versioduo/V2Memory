...
uint32_t V2Memory::EEPROM::getSize(void) {
  ...
  uint32_t virtual_page_size = 4 << psz;
  uint32_t size              = 128 * virtual_page_size;
...
uint32_t V2Memory::EEPROM::getSizeAllocated() {
  const uint8_t sblk = NVMCTRL->SEESTAT.bit.SBLK;
...
void V2Memory::EEPROM::prepareWrite() {
  NVMCTRL->SEECFG.reg = NVMCTRL_SEECFG_WMODE_BUFFERED;
  while (NVMCTRL->SEESTAT.bit.BUSY)
    ;
}

void V2Memory::EEPROM::flush() {
...
void V2Memory::EEPROM::write(uint32_t offset, const uint8_t *buffer, uint32_t size) {
